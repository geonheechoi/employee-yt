...
app.post("/attendance", async (req, res) => {
  try{
    ...
    res.status(500).json({message: "Failed to save the attendance"});
  }
});
...
app.get("/attendance", async (req, res) => {
  try{
    ...
    const attendanceData = await Attendance.find({date :data});
...
app.get("/attendance-report-all-employees", async (req, res) => {
  try{
    ...
    console.log("query papram",month,year);

    const startDate= moment(`${year}-${month}-01`).format("YYYY-MM-DD").startOf("month").toDate();
    ...
    const report = await Attendance.aggregate([
      ...
      },
      {
        $group: {
          ...
          // Assuming 'holiday' should check for "holiday" status
          holiday: {
            $sum: {
